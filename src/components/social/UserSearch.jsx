...
export default function UserSearch({ mobile = false }) {
  ...
  return <div className="relative"><label className="flex items-center gap-2 rounded-full bg-slate-100 px-4 py-2 text-sm text-slate-500"><Search className="h-4 w-4" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar na rede" className={`${mobile ? 'w-full' : 'w-36'} bg-transparent outline-none placeholder:text-slate-500`} /></label>{search && <div className={`${mobile ? 'static mt-3' : 'absolute right-0 top-12'} z-30 w-full rounded-xl border border-slate-200 bg-white p-2 shadow-xl`}>{results.length ? results.map((person) => <SearchResult key={person.id} person={person} />) : <p className="p-3 text-sm text-slate-500">Nenhuma pessoa encontrada.</p>}</div>}</div>;
}