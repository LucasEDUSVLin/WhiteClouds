...
export default function PostCard({ post, currentUserId, onLike, onDelete }) {
  ...
  return <article className={`border-b bg-white p-5 transition hover:bg-slate-50 sm:p-6 ${isOwnPost ? 'border-orange-300 ring-1 ring-inset ring-orange-200' : 'border-slate-200'}`}><div className="flex gap-3"><Avatar name={post.authorName} image={post.authorAvatar} /><div className="min-w-0 flex-1"><div className="flex items-center gap-2 text-sm"><strong className="truncate">{post.authorName || 'Usuário WhiteClouds'}</strong>{isOwnPost && <span className="rounded-full bg-orange-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-orange-700">Você</span>}<span className="truncate text-slate-500">@{post.authorHandle || 'whitecloud'}</span><span className="text-slate-400">· {formatDate(post.createdAt)}</span>{isOwnPost && <button type="button" onClick={() => onDelete(post.id)} className="ml-auto rounded p-1 text-slate-400 hover:bg-red-50 hover:text-red-600" aria-label="Excluir post"><Trash2 className="h-4 w-4" /></button>}</div><p className="mt-2 whitespace-pre-wrap text-[15px] leading-7 text-slate-700">{post.content}</p><div className="mt-4 flex items-center gap-8 text-slate-400"><button type="button" onClick={() => onLike(post)} className={`flex items-center gap-2 text-sm transition ${liked ? 'text-rose-500' : 'hover:text-rose-500'}`}><Heart className={`h-4 w-4 ${liked ? 'fill-current' : ''}`} />{post.likedBy?.length || 0}</button><button type="button" className="text-slate-400 hover:text-indigo-600" aria-label="Mais opções"><MoreHorizontal className="h-4 w-4" /></button></div></div></div></article>;
}